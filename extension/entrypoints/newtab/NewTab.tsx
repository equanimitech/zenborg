...
import { type Run, runs } from "@/modules/domain";
...
function computeStats(
  ...
  return result.sort(
    (a, b) => b.dwellMs + b.audibleDwellMs - (a.dwellMs + a.audibleDwellMs),
  );
}

function groupByArea(
  stats: readonly DomainStats[],
  ...
  for (const [areaId, domains] of groups) {
    const area =
      areaId === "__uncategorized__"
        ? null
        : (allAreas.find((a) => a.id === areaId) ?? null);
    const totalDwell = domains.reduce(
      (sum, d) => sum + d.dwellMs + d.audibleDwellMs,
      0,
    );
...
function actionSummary(s: DomainStats): string {
  const parts: string[] = [];
  if (s.videos > 0) {
    parts.push(`${s.videos} video${s.videos === 1 ? "" : "s"}`);
  }
  if (s.posts > 0) {
    parts.push(`${s.posts} post${s.posts === 1 ? "" : "s"}`);
  }
  if (s.games > 0) {
    parts.push(`${s.games} game${s.games === 1 ? "" : "s"}`);
  }
  return parts.join(", ");
}
...
function groupByPhase(
  moments: readonly TodayMoment[],
): [string, readonly TodayMoment[]][] {
...
export function NewTab() {
  ...
  const [board, setBoard] = useState<TodayBoard>({
    moments: [],
    currentPhase: "",
  });
  ...
  useEffect(() => {
    fenceCache
      .getValue()
      .then((fences) => setFenceCount(Object.keys(fences).length));
    ...
    Promise.all([queryEvents(dayStart), areaMapStore.getValue()])
      .then(([events, map]) => {
        setAreaMap(map);
        const domainRuns = runs(events);
        const computed = computeStats(
          domainRuns,
          events as readonly { kind?: string; payload?: { domain?: string } }[],
          map,
        );
        setStats(computed);
        setLoading(false);
  ...
  const handleAssign = async (
    ...
    setStats(
      stats.map((s) =>
        s.domain === domain ? { ...s, areaId: areaId || undefined } : s,
      ),
    );
  ...
  const maxDwell =
    stats.length > 0 ? stats[0].dwellMs + stats[0].audibleDwellMs : 1;
      ...
      <div className="newtab-center">
        ...
        <div className="newtab-intention">
          {activeMoment ? (
            <p className="newtab-intention-line">
              ...
              {activeMoment.emoji && (
                <span className="newtab-intention-emoji">
                  {activeMoment.emoji}
                </span>
              )}
              <span className="newtab-intention-name">{activeMoment.name}</span>
              {activeMoment.area && (
                <span className="newtab-intention-area">
                  {activeMoment.area}
                </span>
              )}
            </p>
          ...
          <section className="newtab-moments">
            {phaseGroups.map(([phase, moments]) => (
              <div key={phase} className="newtab-phase-group">
                <p
                  className={`newtab-phase-label ${phase === board.currentPhase ? "newtab-phase-current" : ""}`}
                >
                  {phaseLabel(phase)}
                </p>
                <ul className="newtab-moment-list">
                  {moments.map((m) => (
                    ...
                    >
                      <span className="newtab-moment-indicator">
                        {m.active ? "●" : m.status === "DONE" ? "✓" : "○"}
                      </span>
                      {m.areaEmoji && (
                        <span className="newtab-moment-emoji">
                          {m.areaEmoji}
                        </span>
                      )}
                      <span className="newtab-moment-name">{m.name}</span>
                      {m.startTime && (
                        <span className="newtab-moment-time">
            ...
            {areaGroups.map((group) => (
              <div key={group.areaId} className="newtab-area-group">
                <div className="newtab-area-header">
                  ...
                  <span className="newtab-area-dwell">
                    {formatDwell(group.totalDwell)}
                  </span>
                </div>
                <div className="newtab-circles">
                  {group.domains.map((s) => {
                    ...
                    const audibleNote =
                      s.audibleDwellMs > 0
                        ? ` (${formatDwell(s.audibleDwellMs)} audible)`
                        : "";
                    return (
                      <div key={s.domain} className="newtab-circle-wrap">
                        <button type="button"
                          ...
                          style={{
                            ...
                            height: `${size}px`,
                            backgroundColor:
                              group.area?.color ?? "var(--quiet)",
                          }}
                          onClick={() =>
                            setAssigningDomain(isAssigning ? null : s.domain)
                          }
                        ...
                        <span className="newtab-circle-label">
                          {s.domain.replace(
                            /\.com$|\.org$|\.io$|\.tv$|\.net$/,
                            "",
                          )}
                        </span>
                        <span className="newtab-circle-dwell">
                          {formatDwell(totalDwell)}
                        </span>
                        {actions && (
                          <span className="newtab-circle-actions">
                            {actions}
                          </span>
                        )}
                        ...
                        {isAssigning && (
                          <div className="newtab-assign">
                            <select
                              // biome-ignore lint/a11y/noAutofocus: the dropdown opens on an explicit click; focus follows the user's action
                              autoFocus
                              value={s.areaId ?? ""}
                              onChange={(e) =>
                                void handleAssign(s.domain, e.target.value)
                              }
        ...
        {(fenceCount > 0 || cooldownUntil !== null) && (
          <p className="newtab-fence-summary">
            ...
            {fenceCount > 0 && cooldownUntil !== null && " · "}
            {cooldownUntil !== null &&
              `cooldown until ${formatUntil(cooldownUntil)}`}
          </p>
        )}
